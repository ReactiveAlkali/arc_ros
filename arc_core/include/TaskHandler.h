...
protected:
  /// The maximum number of tasks to keep in our backlog
  const int max_backlog = 5; 

  /**
   * How long to wait for a task confirmation
   */
  const ros::Duration confirmation_timeout = ros::Duration(15);
  ...
  /**
   * Custom callback queue for keeping track of task requests and their connection with action 
   * server.
   */
  ...
  /**
   * List of the task requests we've been given but have not started. If we are currently in the 
   * middle of a request, then we put this here.
   */
  std::vector<TaskGoal> task_backlog;

  /**
   * The tasks currently requested and being worked on. When a task is done we remove it from this 
   * list
   */
  std::vector<TaskGoal> active_tasks;

  /**
   * An action client used to coordinate the task assignment process
   */
  actionlib::SimpleActionClient<arc_msgs::TaskCoordinationAction> task_coordination_client;
   ...
   * @param suitability: The suitability of the accepted task
   */
  void acceptRequest(const TaskGoal goal, int suitability);
...
//task handler specific
  ...
  /**
   * Since this object is exposed to many asynchronous connections, the state will change without 
   * it knowing.
...
//SERVER: Routines with the perspective that TaskHandler is a server with respect to 
//  RecruitmentManager
  /**
   * Requests to the TaskHandler go here. This will sift through them and activate the individual
   * tasks.
   *
...
//CLIENT: Routines with the perspective that TaskHandler is a client with respect to TaskServer's
   ...
   */
  void task_handler_result_cb(const actionlib::SimpleClientGoalState& state, 
